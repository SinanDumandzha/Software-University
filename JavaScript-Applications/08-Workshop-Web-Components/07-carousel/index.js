...
class Carousel extends HTMLElement {
    ...
    constructor() {
        ...
        this.itemRenderFn = (item, index) => {
            const willFade =
                this.prevActiveIndex === index && !this.hasFadedSuccessfully;
            const classes = {
    ...
    prevClickHandler() {
        if (this.clickDelayWhileAnimationIsCompleted(this.prevClickHandler)) {
            return;
        }
    ...
    nextClickHandler() {
        if (this.clickDelayWhileAnimationIsCompleted(this.nextClickHandler)) {
            return;
        }
        if (!this.hasFadedSuccessfully) {
            return;
    ...
    dotClickHandler({ target }) {
        ...
        if (
            this.clickDelayWhileAnimationIsCompleted(this.dotClickHandler, {
                target,
            })
        ) {
            return;
