function attachEvents() {
    catchesId = document.querySelector('#catches');
    ...
    async function createCatch() {
        const response = await createResponse(addForm.children, 'POST', 'https://fisher-game.firebaseio.com/catches.json');
        handler(response);
        loadCatches();
        clearAddForm();
    ...
    async function loadCatches() {
        ...
        function getCatches(catchesId) {
            Object.entries(catchesId).forEach(([id, details]) => {
    ...
    function cleaner() {
        while (catchesId.children.length > 0) {
            catchesId.removeChild(catchesId.children[0]);
        }
    }
}
